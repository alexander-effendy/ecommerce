...
const Page = () => {
  ...
  const { startUpload } = useUploadThing("imageUploader", {
    onClientUploadComplete: ([data]) => {
      ...
      startTransition(() => {
        router.push(`/configure/design?id=${configId}`) // after uploading, move to next step with this configId
      })
    },
    onUploadProgress(p) {
      setUploadProgress(p)
  ...
  return (
    <div
      className={cn(
        'relative h-full flex-1 my-16 w-full rounded-xl bg-gray-900/5 p-2 ring-1 ring-inset ring-gray-900/10 lg:grounded-2xl flex justify-center flex-col items-center',
        {
